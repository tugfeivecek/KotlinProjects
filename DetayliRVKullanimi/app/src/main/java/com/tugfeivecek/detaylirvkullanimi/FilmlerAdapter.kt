...
class FilmlerAdapter (private val mContext: Context,private val filmlerListesi:List<Filmler>)
    ...
    inner class CardTasarimNesneleriniTutucu(view: View):RecyclerView.ViewHolder(view){
        ...
        var imageViewFilResim:ImageView
        var textViewFilmBaslik:TextView
        var textViewFilmFiyat:TextView
        var buttonSepeteEkle:Button

        init{
            imageViewFilResim=view.findViewById(R.id.imageViewFilmResim)
    ...
    //veri kumesini tasarimla birlestirdik hangi veriler gorunecek onu belirledik
    // resim alanına yazı ekleme tıklanılma ozellikleri vs.verebılırız
    override fun onBindViewHolder(holder: CardTasarimNesneleriniTutucu, position: Int) {
        ...
        holder.imageViewFilResim.setImageResource(mContext.resources.getIdentifier(film.film_resim_ad,"drawable",mContext.packageName))
